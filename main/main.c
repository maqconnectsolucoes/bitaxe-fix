...
void app_main(void)
{
    ...
    if (system_init_ret == ESP_OK) {
        if (asic_initialize(&GLOBAL_STATE, ASIC_INIT_COLD_BOOT, 0) == 0) {
            ...
            if (xTaskCreateWithCaps(autotune_task, "autotune", 4096, (void *) &GLOBAL_STATE, 2, NULL, MALLOC_CAP_SPIRAM) != pdPASS) {
                ESP_LOGE(TAG, "Error creating autotune task");
            }
            if (xTaskCreateWithCaps(mqtt_task, "mqtt", 8192, (void *) &GLOBAL_STATE, 3, NULL, MALLOC_CAP_SPIRAM) != pdPASS) {
                ESP_LOGE(TAG, "Error creating mqtt task");
            }
        }
    }
