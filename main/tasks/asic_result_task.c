...
void ASIC_result_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        bool had_jobid = active_job_snapshot.jobid != NULL;
        bool had_extranonce2 = active_job_snapshot.extranonce2 != NULL;
        active_job_snapshot.jobid = had_jobid ? strdup(active_job_snapshot.jobid) : NULL;
        active_job_snapshot.extranonce2 = had_extranonce2 ? strdup(active_job_snapshot.extranonce2) : NULL;
        pthread_mutex_unlock(&GLOBAL_STATE->valid_jobs_lock);

        if ((had_jobid && active_job_snapshot.jobid == NULL) || (had_extranonce2 && active_job_snapshot.extranonce2 == NULL))
        {
            ESP_LOGW(TAG, "Failed to allocate job snapshot, dropping nonce (job 0x%02X)", job_id);
            free(active_job_snapshot.jobid);
            free(active_job_snapshot.extranonce2);
            continue;
        }
        bm_job *active_job = &active_job_snapshot;
        // check the nonce difficulty
        double nonce_diff = test_nonce_value(active_job, asic_result->nonce, asic_result->rolled_version, asic_result->midstate_index);
        ...
        if (nonce_diff >= active_job->pool_diff)
        {
            if (GLOBAL_STATE->stratum_protocol == STRATUM_PROTOCOL_V2) {
                ...
                if (stratum_v2_is_extended_channel(GLOBAL_STATE)) {
                    ...
                    // The pool prepends its extranonce_prefix separately. The job
                    // snapshot already carries exactly that many bytes as hex
                    // (create_jobs_task), so derive the length from it instead
                    // of dereferencing sv2_conn, which the SV2 task may have
                    // freed by now; hex2bin is bounded by the buffer size.
                    uint8_t extranonce_2[32];
                    uint8_t en2_len = (uint8_t) hex2bin(active_job->extranonce2, extranonce_2, sizeof(extranonce_2));
