...
esp_err_t scoreboard_add(Scoreboard *scoreboard, double difficulty, const char *job_id, const char *extranonce2, uint32_t ntime, uint32_t nonce, uint32_t version_bits)
{
    ...
    // Track which slots moved so we can persist them to NVS after releasing the mutex: NVS
    // writes are millisecond-scale, and doing up to MAX_SCOREBOARD of them back-to-back while
    // holding the mutex stalled asic_result_task (the caller) as well as any other reader
    // (e.g. the HTTP /api/system/scoreboard handler) for the whole insertion.
    int dirty_indices[MAX_SCOREBOARD];
    int dirty_count = 0;
    ...
    // scoreboard_add() is only ever called from asic_result_task, so entries[] can't change
    // underneath us here even though the mutex is no longer held.
    for (int k = 0; k < dirty_count; k++) {
        int idx = dirty_indices[k];
        scoreboard_save(idx, &scoreboard->entries[idx]);
    }
