...
static void decode_mining_notification(GlobalState * GLOBAL_STATE, const mining_notify *mining_notification)
{
    ...
    // extranonce_str is only ever written by this task (set_extranonce /
    // subscribe result below), so reading it here needs no extranonce_lock.
            ...
            switch (stratum_api_v1_message.method) {
                ...
                case STRATUM_RESULT_CONFIGURE:
                    if (stratum_api_v1_message.response_success) {
                        ...
                        ESP_LOGE(TAG, "Configure result rejected: %s",
                                 stratum_api_v1_message.error_str ? stratum_api_v1_message.error_str : "unknown");
                    }
                    break;
                ...
                case STRATUM_RESULT_SUBSCRIBE:
                    ...
                    {
                        // Swap the pair atomically w.r.t. create_jobs_task, which
                        // copies both fields under the same lock; free the old
                        // string only after the unlock so no reader can still
                        // hold it.
