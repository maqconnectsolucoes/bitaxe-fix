...
// Free a work item with the free function captured for it at enqueue time
// (see work_queue.h). Deciding by the *current* protocol was wrong: the
// coordinator does queue_clear -> stratum_protocol = new -> start task, so an
// item dequeued around a switch may belong to either protocol.
static void free_work(void (*free_fn)(void *), void *work)
{
    if (!work) return;
    if (free_fn) {
        free_fn(work);
...
void create_jobs_task(void *pvParameters)
{
    ...
    void (*current_work_free_fn)(void *) = NULL;
    ...
    while (1) {
        ...
        if (active_protocol != current_work_protocol) {
            if (current_work != NULL) {
                ...
                free_work(current_work_free_fn, current_work);
                current_work = NULL;
            }
        ...
        void (*new_work_free_fn)(void *) = NULL;
        void *new_work = queue_dequeue_timeout_ex(&GLOBAL_STATE->stratum_queue, timeout_ms, &new_work_free_fn);
        ...
        if (new_work != NULL) {
            ...
            if (active_protocol != current_work_protocol) {
                // Protocol switched during our blocking dequeue. The item may be
                // from either protocol, but the queue captured the right free
                // function for it at enqueue time, so nothing leaks (SV2 ext jobs
                // carry up to 64 KB of coinbase prefix/suffix each).
                ESP_LOGW(TAG, "Protocol switch detected during dequeue, discarding stale item");
                free_work(new_work_free_fn, new_work);
                current_work_protocol = active_protocol;
            ...
            current_work_free_fn = new_work_free_fn;
...
static void generate_work(GlobalState *GLOBAL_STATE, mining_notify *notification, uint64_t extranonce_2, double difficulty)
{
    // Copy the (extranonce_str, extranonce_2_len) pair under the lock: the
    // stratum V1 task swaps and frees it on mining.set_extranonce, and the two
    // fields must be read together or the job is built with a mismatched
    // extranonce and silently produces invalid shares.
    char extranonce_1[MAX_EXTRANONCE_STR_LEN + 1];
    ...
    if (GLOBAL_STATE->extranonce_str == NULL) {
        ...
        ESP_LOGW(TAG, "No extranonce yet (notify before subscribe result), skipping job");
        return;
    }
    size_t copied = strlcpy(extranonce_1, GLOBAL_STATE->extranonce_str, sizeof(extranonce_1));
    extranonce_2_len = GLOBAL_STATE->extranonce_2_len;
    pthread_mutex_unlock(&GLOBAL_STATE->extranonce_lock);

    if (copied >= sizeof(extranonce_1)) {
        ESP_LOGE(TAG, "extranonce1 longer than %d chars, skipping job", MAX_EXTRANONCE_STR_LEN);
        return;
    }
    if (extranonce_2_len > MAX_EXTRANONCE2_LEN) {
        ESP_LOGE(TAG, "extranonce_2_len %d exceeds maximum %d, skipping job", extranonce_2_len, MAX_EXTRANONCE2_LEN);
        return;
    }
    ...
    calculate_coinbase_tx_hash(notification->coinbase_1, notification->coinbase_2, extranonce_1, extranonce_2_str, coinbase_tx_hash);
...
{
    // Snapshot the extended-channel parameters under the lock: the SV2 task
    // frees sv2_conn on any pool drop or coordinator shutdown, possibly while
    // this task is still building work from the previous job.
    uint8_t extranonce_2_len;
    uint8_t extranonce_prefix[32];
    uint8_t extranonce_prefix_len;
    pthread_mutex_lock(&GLOBAL_STATE->sv2_conn_lock);
    sv2_conn_t *conn = GLOBAL_STATE->sv2_conn;
    if (!conn) {
        pthread_mutex_unlock(&GLOBAL_STATE->sv2_conn_lock);
        return;
    }
    // SV2 spec: extranonce_size is the miner's rollable portion (not total)
    extranonce_2_len = conn->extranonce_size;
    extranonce_prefix_len = conn->extranonce_prefix_len;
    memcpy(extranonce_prefix, conn->extranonce_prefix, sizeof(extranonce_prefix));
    pthread_mutex_unlock(&GLOBAL_STATE->sv2_conn_lock);
    conn = NULL; // must not be touched past this point
    ...
    if (extranonce_2_len > sizeof(extranonce_2)) {
        extranonce_2_len = sizeof(extranonce_2);
    }
