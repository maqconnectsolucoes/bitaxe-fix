                            ...
                            uint32_t ntime, uint32_t version)
{
    ...
    // Copy channel_id and claim a sequence number under the lock, then send
    // outside it: the SV2 task may free sv2_conn at any point.
    ...
    uint32_t channel_id = conn->channel_id;
...
{
    // Flat flag maintained alongside sv2_conn: callers on other tasks must
    // not dereference sv2_conn, which this task frees on any pool drop.
    return GLOBAL_STATE->sv2_extended_channel;
}

// Unpublish the connection before freeing it. Readers take sv2_conn_lock,
// re-check sv2_conn and copy what they need, so once the pointer is NULL
// under the lock nobody can still be reading the struct we free.
static void sv2_conn_release(GlobalState *GLOBAL_STATE, sv2_conn_t *conn)
{
    pthread_mutex_lock(&GLOBAL_STATE->sv2_conn_lock);
...
void stratum_v2_task(void *pvParameters)
{
    ...
    if (!frame_buf || !recv_buf) {
        ...
        free(frame_buf);
        free(recv_buf);
        sv2_conn_release(GLOBAL_STATE, conn);
        protocol_coordinator_notify_failure();
        vTaskDelete(NULL);
        return;
    }
    ...
    while (1) {
        ...
        GLOBAL_STATE->sv2_extended_channel = (channel_type == SV2_CHANNEL_EXTENDED);
        ...
        // 4. Receive OpenMiningChannelSuccess
        {
            ...
            if (channel_type == SV2_CHANNEL_EXTENDED) {
                ...
                if (extranonce_size > sizeof(conn->extranonce_prefix)) {
                    ESP_LOGW(TAG, "Extranonce_size %u exceeds maximum %u, clamping to maximum",
                             extranonce_size, (unsigned)sizeof(conn->extranonce_prefix));
                    extranonce_size = sizeof(conn->extranonce_prefix);
                }
