...
static const char* HEAP_TAG = "heap_monitor";
...
// Periodically logs heap/PSRAM free size (current + all-time low water mark) to help
// correlate slow leaks/fragmentation with the spontaneous reboots that heap_alloc_failed_hook()
// (main.c) triggers via abort() when a PSRAM allocation fails. Runs unconditionally (independent
// of CONFIG_FREERTOS_GENERATE_RUN_TIME_STATS) since this task is always created from main.c.
static void log_heap_health(void)
{
    static uint32_t ticks_since_log = 0;
    if (++ticks_since_log * 1000 < HEAP_LOG_INTERVAL_MS) {
        return;
    }
    ticks_since_log = 0;

    ESP_LOGI(HEAP_TAG, "heap free=%" PRIu32 " min_free=%" PRIu32 " | psram free=%" PRIu32 " min_free=%" PRIu32,
