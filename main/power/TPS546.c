...
// VOUT_MODE is a fixed hardware register (never written by us) read once during
// init and reused for every ULINEAR16<->float conversion. Caching it means a
// transient I2C failure during a later conversion can't leave the exponent
// reading uninitialized stack garbage while encoding a real voltage command.
static uint8_t s_vout_mode = 0;
static bool s_vout_mode_valid = false;
...
static float ulinear16_2_float(uint16_t value)
{
    ...
    if (!s_vout_mode_valid) {
        ESP_LOGE(TAG, "VOUT_MODE not initialized, cannot convert ULINEAR16 value");
        return 0.0f;
    }

    if (s_vout_mode & 0x10) {
        // exponent is negative
        exponent = -1 * ((~s_vout_mode & 0x1F) + 1);
    } else {
        exponent = (s_vout_mode & 0x1F);
    }
...
static uint16_t float_2_ulinear16(float value)
{
    ...
    if (!s_vout_mode_valid) {
        ESP_LOGE(TAG, "VOUT_MODE not initialized, refusing to encode Vout command");
        return 0;
    }
...
esp_err_t TPS546_init(TPS546_CONFIG config)
{
    ...
    // VOUT_MODE is fixed by the hardware and never written by us - read it once here
    // (with retries, like the device ID above) and cache it. Every later voltage
    // conversion (including the ones TPS546_write_entire_config() is about to trigger)
    // relies on this being valid, so a persistent I2C failure here must abort init
    // rather than let the exponent default to uninitialized/zero.
    s_vout_mode_valid = false;
    for (int attempt = 0; attempt < max_attempts; ++attempt) {
        if (smb_read_byte(PMBUS_VOUT_MODE, &s_vout_mode) == ESP_OK) {
    ...
    if (!s_vout_mode_valid) {
        ESP_LOGE(TAG, "Could not read VOUT_MODE from TPS546");
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "VOUT_MODE: %02x", s_vout_mode);
...
// Consecutive failed PMBus reads across the getters below. They deliberately
// return the last good value on a glitch, but the consumer must be able to
// tell "fresh" from "stale for the last N seconds" - see
// TPS546_get_read_failures().
static uint32_t read_failures = 0;

uint32_t TPS546_get_read_failures(void)
{
