...
typedef struct
{
    ...
    // Guards the (extranonce_str, extranonce_2_len) pair: the stratum V1 task
    // swaps and frees it on mining.set_extranonce while create_jobs_task is
    // building work from it on another core.
    ...
    // Guards publication and teardown of sv2_conn: readers on other tasks
    // (asic_result_task, create_jobs_task) take it, re-check sv2_conn and
    // copy the fields they need, so the SV2 task can free the struct without
    // anyone still dereferencing it.
    pthread_mutex_t sv2_conn_lock;
    // Flat copy of "sv2_conn->channel_type == SV2_CHANNEL_EXTENDED", so the
    // hot path can ask without touching sv2_conn at all.
