        ...
        case BAP_PARAM_FREQUENCY:
            {
                ...
                if (!bap_global_state) {
                    BAP_send_message(BAP_CMD_ERR, parameter, "not_ready");
                    return;
                }
                bool overclock = nvs_config_get_bool(NVS_CONFIG_OVERCLOCK_ENABLED);
                if (!tuning_frequency_allowed(bap_global_state, overclock, target_frequency)) {
                    asic_range_t range = tuning_frequency_range(bap_global_state, overclock);
                    ESP_LOGE(TAG, "Invalid frequency value: %.2f MHz (valid range: %u-%u MHz)", target_frequency, range.min, range.max);
        ...
        case BAP_PARAM_ASIC_VOLTAGE:
            {
                ...
                bool overclock = nvs_config_get_bool(NVS_CONFIG_OVERCLOCK_ENABLED);
                if (!tuning_voltage_allowed(bap_global_state, overclock, target_voltage_mv)) {
                    asic_range_t range = tuning_voltage_range(bap_global_state, overclock);
                    ESP_LOGE(TAG, "Invalid voltage value: %d mV (valid range: %u-%u mV)", target_voltage_mv, range.min, range.max);
                    BAP_send_message(BAP_CMD_ERR, parameter, "invalid_range");
                    return;
                }
