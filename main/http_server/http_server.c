...
// GET_system_statistics only takes "columns=<comma list of the labels below>"
// (19 labels, longest 15 chars), so this comfortably fits any legitimate query.
#define STATS_QUERY_MAX 256
...
static esp_err_t GET_system_coredump(httpd_req_t *req)
{
    if (is_network_allowed(req) != ESP_OK) {
        return httpd_resp_send_err(req, HTTPD_401_UNAUTHORIZED, "Unauthorized");
    }
    ...
    if (err != ESP_OK || size == 0) {
        // Estado normal de um aparelho saudável: sem panic desde o último apagamento.
        ESP_LOGI(TAG, "No core dump stored (%s)", esp_err_to_name(err));
        return httpd_resp_send_err(req, HTTPD_404_NOT_FOUND, "No core dump stored");
    }

    const esp_partition_t *part =
        esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_COREDUMP, NULL);
    if (part == NULL) {
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Core dump partition not found");
    }
    // esp_core_dump_image_get devolve endereço absoluto de flash; esp_partition_read
    // espera deslocamento dentro da partição.
    if (addr < part->address || (addr - part->address) + size > part->size) {
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Core dump outside its partition");
    }
    size_t offset = addr - part->address;

    ESP_LOGI(TAG, "Serving core dump: %u bytes", (unsigned) size);
    httpd_resp_set_type(req, "application/octet-stream");
    httpd_resp_set_hdr(req, "Content-Disposition", "attachment; filename=\"coredump.bin\"");

    char chunk[1024];
    size_t restante = size;
    esp_err_t res = ESP_OK;

    while (restante > 0) {
        size_t n = restante < sizeof(chunk) ? restante : sizeof(chunk);
        res = esp_partition_read(part, offset, chunk, n);
        if (res != ESP_OK) {
            ESP_LOGE(TAG, "Failed reading core dump: %s", esp_err_to_name(res));
            break;
        }
        res = httpd_resp_send_chunk(req, chunk, n);
        if (res != ESP_OK) {
            ESP_LOGE(TAG, "Failed sending core dump chunk: %s", esp_err_to_name(res));
            break;
        }
        offset += n;
        restante -= n;
...
// Apagar é um passo separado e explícito: sem isso não há como distinguir um
// dump novo de um que já foi analisado, já que o próximo panic só grava se a
// partição estiver livre.
static esp_err_t DELETE_system_coredump(httpd_req_t *req)
{
    if (is_network_allowed(req) != ESP_OK) {
        return httpd_resp_send_err(req, HTTPD_401_UNAUTHORIZED, "Unauthorized");
    }
...
// Literal que o AxeOS envia por um campo de senha que o usuário não editou.
...
static void update_pool_nvs(const cJSON *pool_item, int i) {
    ...
    // Only an explicit string replaces the stored password. AxeOS sends the
    // '*****' placeholder for an untouched field, and a client may omit the
    // field altogether; both mean "keep what is stored" (fallback "x" only when
    // nothing is stored yet).
    if (new_pass && cJSON_IsString(new_pass) && strcmp(new_pass->valuestring, PASSWORD_PLACEHOLDER) != 0) {
...
bool check_settings_and_update(const cJSON * const root, char **redirect_url)
{
    ...
    for (NvsConfigKey key = 0; key < NVS_CONFIG_COUNT; key++) {
        ...
        // The generic min/max above only rules out absurd values; the real
        // envelope depends on the chip (preset tables + absolute overclock
        // ceiling). overclockEnabled may arrive in the same body, so honour
        // that before falling back to what is persisted.
        if ((key == NVS_CONFIG_ASIC_VOLTAGE || key == NVS_CONFIG_ASIC_FREQUENCY) && cJSON_IsNumber(item)) {
            cJSON * oc_item = cJSON_GetObjectItem(root, "overclockEnabled");
            bool overclock;
            if (oc_item && cJSON_IsBool(oc_item)) {
                overclock = cJSON_IsTrue(oc_item);
            } else if (oc_item && cJSON_IsNumber(oc_item)) {
                overclock = oc_item->valueint != 0;
            } else {
                overclock = nvs_config_get_bool(NVS_CONFIG_OVERCLOCK_ENABLED);
            }
            bool allowed = (key == NVS_CONFIG_ASIC_VOLTAGE)
                               ? tuning_voltage_allowed(GLOBAL_STATE, overclock, (uint16_t) item->valueint)
                               : tuning_frequency_allowed(GLOBAL_STATE, overclock, (float) item->valuedouble);
            if (!allowed) {
                asic_range_t range = (key == NVS_CONFIG_ASIC_VOLTAGE) ? tuning_voltage_range(GLOBAL_STATE, overclock)
                                                                      : tuning_frequency_range(GLOBAL_STATE, overclock);
                ESP_LOGW(TAG, "Value '%g' for '%s' is outside the %s envelope [%u, %u] (overclock %s)", item->valuedouble,
                         setting->rest_name, GLOBAL_STATE->DEVICE_CONFIG.family.asic.name, range.min, range.max,
                         overclock ? "on" : "off");
                result = false;
            }
        }
        ...
        for (NvsConfigKey key = 0; key < NVS_CONFIG_COUNT; key++) {
            ...
            switch(setting->type) {
                case TYPE_STR:
                    ...
                    else if (key == NVS_CONFIG_MQTT_PASS && strcmp(item->valuestring, PASSWORD_PLACEHOLDER) == 0)
                    {
                        // AxeOS devolve o placeholder quando o usuário não tocou no campo;
                        // gravá-lo literalmente apagaria a senha do broker (era o que as
                        // senhas de pool faziam antes de update_pool_nvs ser corrigida).
                    }
                    else
                    {
...
static esp_err_t GET_system_statistics(httpd_req_t * req)
{
    ...
    // Check query parameters. Fixed buffers: the previous three query-sized
    // VLAs (up to CONFIG_HTTPD_MAX_URI_LEN = 2048 each) all lived at once on
    // the 8 KB HTTPD task stack, so a long query could smash it.
    if (bufLen > STATS_QUERY_MAX) {
        return httpd_resp_send_err(req, HTTPD_414_URI_TOO_LONG, "Query too long");
    }
    if (1 < bufLen) {
        char buf[STATS_QUERY_MAX];
        if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
            char columns[STATS_QUERY_MAX];
            if (httpd_query_key_value(buf, "columns", columns, sizeof(columns)) == ESP_OK) {
                // The raw query is no longer needed: decode into its buffer.
                url_decode(buf, columns);
...
esp_err_t start_rest_server(void * pvParameters)
{
    ...
    // Let the TCP stack detect and drop sockets held by clients that went away
    // without closing (sleeping phone, dropped Wi-Fi). Without this they linger
    // until max_open_sockets fills up and lru_purge starts evicting live
    // connections, which the evicted browsers answer by reconnecting at once.
    ...
    httpd_uri_t system_coredump_get_uri = {
        ...
        .method = HTTP_GET,
        .handler = GET_system_coredump,
        .user_ctx = rest_context
    };
    httpd_register_uri_handler(server, &system_coredump_get_uri);

    httpd_uri_t system_coredump_delete_uri = {
        .uri = "/api/system/coredump",
        .method = HTTP_DELETE,
        .handler = DELETE_system_coredump,
        .user_ctx = rest_context
    };
    httpd_register_uri_handler(server, &system_coredump_delete_uri);
