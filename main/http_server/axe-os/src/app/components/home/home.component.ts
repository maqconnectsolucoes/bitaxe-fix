  ...
  private startGetLiveData() {
    this.info$ = this.liveDataService.info$.pipe(
      ...
      map(info => {
        ...
        // Partial WS pushes can arrive before these fields exist (e.g. before the initial
        // REST fetch resolves), so guard against undefined/null rather than crashing the
        // shareReplay'd stream for every subscriber until a page reload.
        formatted.power = parseFloat((formatted.power ?? 0).toFixed(1));
        formatted.voltage = parseFloat((formatted.voltage ?? 0).toFixed(1));
        formatted.current = parseFloat((formatted.current ?? 0).toFixed(1));
        formatted.coreVoltageActual = parseFloat((formatted.coreVoltageActual ?? 0).toFixed(2));
        formatted.coreVoltage = parseFloat((formatted.coreVoltage ?? 0).toFixed(2));
        formatted.temp = parseFloat((formatted.temp ?? 0).toFixed(1));
        formatted.temp2 = parseFloat((formatted.temp2 ?? 0).toFixed(1));
        formatted.responseTime = parseFloat((formatted.responseTime ?? 0).toFixed(1));
