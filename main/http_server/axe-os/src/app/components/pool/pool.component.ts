...
import { base58Validator } from 'src/app/validators/base58.validator';
...
export class PoolComponent implements OnInit, OnDestroy {
      ...
      .subscribe(info => {
        ...
        const poolsFormGroups = poolsList.map((pool: any) => {
          ...
          return this.fb.group({
            ...
            stratumV2AuthorityPubkey: [pool.stratumV2AuthorityPubkey || '', [base58Validator()]]
          });
        });
        ...
        this.form.get('primaryPoolIndex')?.valueChanges.pipe(takeUntil(this.destroy$)).subscribe(primVal => {
        ...
        this.form.get('secondaryPoolIndex')?.valueChanges.pipe(takeUntil(this.destroy$)).subscribe(secVal => {
  ...
  setupTlsValidationForIndex(index: number) {
    ...
    poolGroup.get('stratumTLS')?.valueChanges.pipe(takeUntil(this.destroy$)).subscribe(value => {
  ...
  addPool() {
    if (this.poolsArray.length < 8) {
      ...
      const poolGroup = this.fb.group({
        ...
        stratumV2ChannelType: ['extended'],
        stratumV2AuthorityPubkey: ['', [base58Validator()]]
      });

      poolGroup.markAsDirty();
  ...
  public updateSystem() {
    // Untouched password fields keep the '*****' placeholder on purpose: the
    // firmware treats that literal as "keep the stored password".
    //
    // update_pool_nvs() rewrites every field of each pool object it receives
    // (missing fields fall back to defaults), so a pool goes as a whole object
    // - but only the pools the user actually edited are sent.
    const raw = this.form.getRawValue();
    const form: Record<string, any> = {};
    const dirtyPools = this.poolsArray.controls
      .map((control, index) => ({ control, index }))
      .filter(({ control }) => control.dirty)
      .map(({ index }) => raw.pools[index]);
    if (dirtyPools.length) {
      form['pools'] = dirtyPools;
    }
    for (const field of ['primaryPoolIndex', 'secondaryPoolIndex']) {
