...
import { Observable, Subject, merge, switchMap, map, shareReplay, timer, takeUntil, finalize, fromEvent, startWith, distinctUntilChanged } from 'rxjs';
...
export class ScoreboardComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // The scoreboard (top-20 all-time diffs) changes rarely, so poll far less often while the
    // tab is hidden instead of hitting the embedded HTTP server every 5s regardless of visibility.
    const visibility$ = fromEvent(document, 'visibilitychange').pipe(
      map(() => document.visibilityState),
      startWith(document.visibilityState),
      distinctUntilChanged()
    );
    const visibilityPolling$ = visibility$.pipe(
      switchMap(state => {
        const interval = state === 'visible' ? 5000 : 60000;
        return timer(0, interval); // fire immediately, then on every interval
      })
    );

    this.scoreboard$ = merge(visibilityPolling$, this.refresh$).pipe(
  ...
  // scoreboard$ emits a fresh array every poll; without trackBy every row is
  // torn down and rebuilt each 5 s even when nothing changed.
  trackByRank(_index: number, entry: ISystemScoreboardEntry & { rank: number }): number {
    return entry.rank;
  }
