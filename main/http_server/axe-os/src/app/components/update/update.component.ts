...
export class UpdateComponent {
  ...
  public latestWwwAsset$: Observable<any>;
  ...
  constructor(
    ...
    // A release is published before its binaries finish uploading, so the
    // assets can legitimately be missing for a while. Resolve each one
    // separately to tell "not built yet" apart from an empty list.
    this.latestEspMinerAsset$ = this.latestRelease$.pipe(
      map((release: any) => release?.assets?.find((asset: any) => asset.name === 'esp-miner.bin'))
    );
    this.latestWwwAsset$ = this.latestRelease$.pipe(
      map((release: any) => release?.assets?.find((asset: any) => asset.name === 'www.bin'))
    );
  ...
  public simpleMarkdownParser(markdown: string): string {
    ...
    // GitHub release notes are external content; sanitize explicitly rather than relying
    // solely on the implicit sanitization Angular applies to [innerHTML] bindings.
    return this.sanitizer.sanitize(SecurityContext.HTML, toHTML.trim()) ?? '';
  }
