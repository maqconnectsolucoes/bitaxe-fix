...
import { EMPTY, forkJoin, fromEvent, catchError, distinctUntilChanged, from, map, mergeMap, of, startWith, switchMap, take, timeout, timer, toArray, Observable, Subscription } from 'rxjs';
...
import { parseSuffixString } from 'src/app/utils/suffix';
...
// Devices queried in parallel (2 requests each). The device serving AxeOS has
// max_open_sockets=20 with LRU purge: the old 128 (256 sockets) evicted the live
// WebSocket of the very session running the scan.
const SCAN_CONCURRENCY = 16;
const REFRESH_CONCURRENCY = 6;

// The scan covers the /24 of the serving device.
const SCAN_SUBNET_MASK = '255.255.255.0';
...
export class SwarmComponent implements OnInit, OnDestroy {
  ...
  // filteredSwarm/deviceFamilies are recomputed only when the swarm or the
  // filter changes (see recomputeViews), not on every change-detection pass.
  private _swarm: any[] = [];
  public filteredSwarm: any[] = [];
  ...
  constructor(
    ...
    this.refreshIntervalSubscription = this.refreshIntervalControl.valueChanges.subscribe(value => {
  ...
  ngOnInit(): void {
    ...
    // Countdown only ticks while the tab is visible (same pattern as the
    // scoreboard polling): no refresh traffic from a backgrounded tab.
    const visibility$ = fromEvent(document, 'visibilitychange').pipe(
      map(() => document.visibilityState),
      startWith(document.visibilityState),
      distinctUntilChanged()
    );
    this.countdownSubscription = visibility$.pipe(
      switchMap(state => state === 'visible' ? timer(1000, 1000) : EMPTY)
    ).subscribe(() => {
  ...
  private saveSwarmData() {
    // Cache is only used to redraw the list before the next refreshList() completes, so it
    // doesn't need the peer's full SystemInfo payload persisted to localStorage indefinitely.
    // Drop fields that reveal more about the user's network/pool than the UI needs cached.
    const sanitized = this.swarm.map(device => {
      const { ssid, stratumUser, fallbackStratumUser, macAddr, wifiStatus, wifiRSSI, ...rest } = device as any;
      return rest;
    });
    this.localStorageService.setObject(SWARM_DATA, sanitized);
...
private isIpAddress(value: string): boolean {
  ...
  // refreshList()/scanNetwork() replace `swarm` with freshly-merged device objects on every
  // tick; without trackBy, Angular tears down and rebuilds every card/row in the whole list
  // instead of patching changed fields.
  trackByAddress(_index: number, device: SwarmDevice): string {
    return device.connectionAddress || device.address;
  }
  ...
  scanNetwork() {
    ...
    if (this.isIpAddress(window.location.hostname)) {
      ...
      const { start, end } = this.calculateIpRange(window.location.hostname, SCAN_SUBNET_MASK);
      ...
      this.httpClient.get(`http://${window.location.hostname}/api/system/info`)
        .subscribe({
          next: (response: any) => {
            ...
            const { start, end } = this.calculateIpRange(serverIp, SCAN_SUBNET_MASK);
            const ips = Array.from({ length: end - start + 1 }, (_, i) => this.intToIp(start + i));
            this.performNetworkScan(ips);
          },
          error: () => {
  ...
  private performNetworkScan(ips: string[]) {
    this.getAllDeviceInfo(ips, () => of(null)).subscribe({
      ...
      error: (err) => {
        // Per-device errors are already swallowed above; this only fires on a
        // programming error, but it must not leave the "Scanning" state stuck.
        console.error('Network scan failed', err);
        this.toastr.error('Network scan failed.');
        this.scanning = false;
  ...
  // Bitforge doesn't report the station IP on /api/system/info at all, so fall
  // back to its /api/ap/info endpoint before giving up on the device.
  ...
  private getAllDeviceInfo(addresses: string[], errorHandler: (error: any, address: string) => Observable<SwarmDevice[] | null>, fetchAsic: boolean = true, concurrency: number = SCAN_CONCURRENCY) {
  ...
  public add() {
    ...
    forkJoin({
      ...
      error: (err: HttpErrorResponse) => {
        this.toastr.error(`Could not add device at ${address}. ${err.message}`);
      }
    });
  }
  ...
  public refreshList(fetchAsic: boolean = true) {
    ...
    this.getAllDeviceInfo(addresses, this.refreshErrorHandler, fetchAsic, REFRESH_CONCURRENCY).subscribe({
      next: (result) => {
        this.swarm = result;
        this.sortSwarm();
        this.saveSwarmData();
  ...
  private recomputeViews() {
    ...
    this.filteredSwarm = !filter
      ? this._swarm
      : this._swarm.filter(axe =>
    ...
    for (const axe of this.filteredSwarm) {
      if (!axe) continue;
      const key = `${axe.deviceModel}|${axe.ASICModel}|${axe.asicCount}`;
      if (!families.has(key)) families.set(key, axe);
    }
    this.deviceFamilies = [...families.values()];
  }
  ...
  private numerizeDeviceBestDiffs(info: ISystemInfo) {
    const parseAsNumber = (val: number | string): number => {
      return typeof val === 'string' ? parseSuffixString(val) : val;
    };
