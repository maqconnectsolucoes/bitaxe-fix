...
static void system_api_add_config(cJSON *root, GlobalState *g) {
    ...
    cJSON_AddFloatToObject(root, "autotuneFrequency", nvs_config_get_float(NVS_CONFIG_AUTOTUNE_FREQUENCY));
    ...
    // MQTT. A senha nunca sai daqui: o que vai é o placeholder quando há algo
    // gravado, para o AxeOS mostrar o campo preenchido sem expor o segredo.
    cJSON_AddNumberToObject(root, "mqttEnabled", nvs_config_get_bool(NVS_CONFIG_MQTT_ENABLED) ? 1 : 0);
    char *mqtt_host = nvs_config_get_string(NVS_CONFIG_MQTT_HOST);
    cJSON_AddStringToObject(root, "mqttHost", mqtt_host ? mqtt_host : "");
    free(mqtt_host);
    cJSON_AddNumberToObject(root, "mqttPort", nvs_config_get_u16(NVS_CONFIG_MQTT_PORT));
    char *mqtt_user = nvs_config_get_string(NVS_CONFIG_MQTT_USER);
    cJSON_AddStringToObject(root, "mqttUser", mqtt_user ? mqtt_user : "");
    free(mqtt_user);
    char *mqtt_pass = nvs_config_get_string(NVS_CONFIG_MQTT_PASS);
    cJSON_AddStringToObject(root, "mqttPass", (mqtt_pass && mqtt_pass[0]) ? "*****" : "");
    free(mqtt_pass);
    char *mqtt_ca = nvs_config_get_string(NVS_CONFIG_MQTT_CA_CERT);
    cJSON_AddStringToObject(root, "mqttCaCert", mqtt_ca ? mqtt_ca : "");
    free(mqtt_ca);
    cJSON_AddNumberToObject(root, "mqttInterval", nvs_config_get_u16(NVS_CONFIG_MQTT_INTERVAL));
    // Sempre devolve um valor: quando nada foi gravado, o padrão derivado do MAC
    // já chega preenchido na tela, e o operador pode editar por cima.
    char mqtt_device_id[MQTT_DEVICE_ID_MAX_LEN];
    mqtt_get_device_id(mqtt_device_id, sizeof(mqtt_device_id));
    cJSON_AddStringToObject(root, "mqttDeviceId", mqtt_device_id);
}
