...
esp_netif_t * wifi_init_sta(const char * wifi_ssid, const char * wifi_pass)
{
    ...
    */
    ...
    // Some newer Wi-Fi 6/7 routers running WPA2/WPA3 mixed mode fail the WPA3
    // handshake with a vague error instead of falling back, leaving the miner
    // unable to associate at all. Wiring up the SAE settings (the macros above
    // were defined but never used) makes H2E follow the sdkconfig choice
    // instead of the driver default.
    //
    // Upstream v2.15.1 (0da3a22f) also sets .disable_wpa3_compatible_mode = 1
    // here, but that field only exists from ESP-IDF v6.0; this tree builds
    // against v5.5.3, so it is left out until the toolchain is bumped.
