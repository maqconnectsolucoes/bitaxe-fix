...
*/
static char * parse_dns_name(char * raw_name, char * raw_name_end, char * parsed_name, size_t parsed_name_max_len)
{
    ...
    do {
        if (label >= raw_name_end) {
            return NULL;
        }
        // Read as unsigned so a high-bit label byte (e.g. a DNS compression
        // pointer marker, unsupported here) can't sign-extend into a huge
        // negative value and bypass the length check below.
        int sub_name_len = (uint8_t) *label;
        if (label + 1 + sub_name_len >= raw_name_end) {
            return NULL;
        }
...
static int parse_dns_request(char * req, size_t req_len, char * dns_reply, size_t dns_reply_max_len, dns_server_handle_t h)
{
    ...
    // The header is read through a struct cast below; anything shorter is
    // not a DNS query.
    ...
    for (int qd_i = 0; qd_i < qd_count; qd_i++) {
        char * name_end_ptr = parse_dns_name(cur_qd_ptr, dns_reply + req_len, name, sizeof(name));
        if (name_end_ptr == NULL) {
            ESP_LOGE(TAG, "Failed to parse DNS question: %s", cur_qd_ptr);
            return -1;
        }

        // parse_dns_name only guarantees the *name* ended inside the datagram;
        // the 4-byte type/class that follows must fit too, or we read (and
        // echo back in the answer) bytes past what was received.
        if (name_end_ptr + sizeof(dns_question_t) > dns_reply + req_len) {
            ESP_LOGE(TAG, "DNS question truncated after name: %s", name);
            return -1;
        }
