...
TEST_CASE("Test nonce diff checking", "[mining test_nonce][not-on-qemu]")
{
    ...
    double diff = test_nonce_value(&job, nonce, rolled_version, 0);
    TEST_ASSERT_EQUAL_INT(18, (int)diff);

    // Same nonce, but force the fallback path (no matching midstate) - must
    // agree with the resumed-midstate result above.
    double diff_fallback = test_nonce_value(&job, nonce, rolled_version, job.num_midstates);
    TEST_ASSERT_EQUAL_INT(18, (int)diff_fallback);
}
...
TEST_CASE("Test nonce diff checking with version rolling midstate selection", "[mining test_nonce][not-on-qemu]")
{
    mining_notify notify_message;
    notify_message.prev_block_hash = "d02b10fc0d4711eae1a805af50a8a83312a2215e00017f2b0000000000000000";
    notify_message.version = 0x20000004;
    ...
    // For each of the 4 precomputed midstates, the resumed-midstate path
    // (correct index) must agree with the full-header fallback path.
    uint32_t rolled_version = job.version;
    for (uint8_t i = 0; i < job.num_midstates; i++) {
        double diff_resumed = test_nonce_value(&job, nonce, rolled_version, i);
        double diff_fallback = test_nonce_value(&job, nonce, rolled_version, job.num_midstates);
        TEST_ASSERT_EQUAL_INT((int)diff_fallback, (int)diff_resumed);
