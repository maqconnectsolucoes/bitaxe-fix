...
// --- extranonce1 length limit (create_jobs_task copies it into a fixed buffer) ---

#define HEX32 "0123456789abcdef0123456789abcdef"

TEST_CASE("Parse stratum mining.subscribe accepts a 64-char extranonce1", "[mining.subscribe]")
{
    StratumApiV1Message stratum_api_v1_message = {};
    const char *json_string = "{\"result\":[[[\"mining.notify\",\"abc\"]],\"" HEX32 HEX32 "\",8],\"id\":2,\"error\":null}";
    TEST_ASSERT_TRUE(STRATUM_V1_parse(&stratum_api_v1_message, json_string));
    TEST_ASSERT_EQUAL_INT(64, strlen(stratum_api_v1_message.extranonce_str));
}

TEST_CASE("Parse stratum mining.subscribe rejects a 65-char extranonce1", "[mining.subscribe]")
{
    StratumApiV1Message stratum_api_v1_message = {};
    const char *json_string = "{\"result\":[[[\"mining.notify\",\"abc\"]],\"" HEX32 HEX32 "0\",8],\"id\":2,\"error\":null}";
    TEST_ASSERT_FALSE(STRATUM_V1_parse(&stratum_api_v1_message, json_string));
}

TEST_CASE("Parse stratum mining.set_extranonce rejects a 65-char extranonce1", "[stratum]")
{
    StratumApiV1Message stratum_api_v1_message = {};
    const char *json_string = "{\"id\":1,\"method\":\"mining.set_extranonce\",\"params\":[\"" HEX32 HEX32 "0\",8]}";
    TEST_ASSERT_FALSE(STRATUM_V1_parse(&stratum_api_v1_message, json_string));
}
