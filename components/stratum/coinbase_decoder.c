...
uint64_t coinbase_decode_varint(const uint8_t *data, int data_len, int *offset) {
    if (*offset >= data_len) {
        *offset = data_len;
        return 0;
    }
    ...
    int extra_bytes = (first_byte == 0xFD) ? 2 : (first_byte == 0xFE) ? 4 : 8;

    // The pool-supplied marker byte can claim more length bytes than actually
    // remain in the buffer (e.g. 0xFF wanting 8 more bytes with only 1 left) -
    // clamp to data_len so callers' existing "offset >= len" guards catch it
    // on the next check instead of reading past coinbase_2_bin's allocation.
    if (*offset + extra_bytes > data_len) {
        *offset = data_len;
        return 0;
    }
...
esp_err_t coinbase_process_notification(const mining_notify *notification,
    ...
    for (uint64_t i = 0; i < num_outputs && offset < coinbase_2_len; i++) {
        ...
        // script_len is an attacker-controlled uint64_t (up to UINT64_MAX via the 0xFF
        // varint form) - comparing as "offset + script_len > coinbase_2_len" promotes
        // offset to uint64_t and can wrap around, defeating the bounds check. offset is
        // always <= coinbase_2_len here, so compute the remaining capacity first instead.
        ...
        if (decode_coinbase_tx) {
            if (value_satoshis > 0) {            
                ...
                bool is_user_address = user_address && strncmp(user_address, output_address, strlen(output_address)) == 0;
