...
// Caps how much unterminated data we'll buffer waiting for a '\n'. A pool (or a MITM on the
// stratum TCP connection) that streams data without ever sending a newline would otherwise grow
// json_rpc_buffer without bound until realloc() fails and forces an esp_restart().
#define MAX_JSON_RPC_LINE_LEN (16 * 1024)
...
char * STRATUM_V1_receive_jsonrpc_line(esp_transport_handle_t transport)
{
    ...
    while (!strstr(json_rpc_buffer, "\n")) {
        ...
        if (nbytes > 0) {
            ...
            if (strlen(json_rpc_buffer) > MAX_JSON_RPC_LINE_LEN) {
                ESP_LOGE(TAG, "Error: JSON-RPC line exceeded %d bytes without a newline, dropping connection",
                         MAX_JSON_RPC_LINE_LEN);
                free(json_rpc_buffer);
                json_rpc_buffer = NULL;
...
static bool parse_mining_notify(cJSON *json, StratumApiV1Message *message)
{
    ...
    if (!job_id_item || !cJSON_IsString(job_id_item) ||
        !prev_block_hash_item || !cJSON_IsString(prev_block_hash_item) ||
        !coinbase_1_item || !cJSON_IsString(coinbase_1_item) ||
        !coinbase_2_item || !cJSON_IsString(coinbase_2_item) ||
        ...
        ESP_LOGE(TAG, "Invalid or wrong-typed params in mining.notify");
        return false;
    }
    ...
    for (size_t i = 0; i < n_merkle_branches; i++) {
        ...
        if (!branch_item || !cJSON_IsString(branch_item)) {
            ESP_LOGE(TAG, "Invalid merkle_branch entry %zu in mining.notify", i);
            return false;
        }
    }
    ...
    // calloc (not malloc): entries shorter than a full hash leave the rest of the slot
    // zeroed instead of uninitialized heap data.
    new_work->merkle_branches = calloc(n_merkle_branches, HASH_SIZE);

    if (!new_work->job_id || !new_work->prev_block_hash || !new_work->coinbase_1 || !new_work->coinbase_2 ||
        (n_merkle_branches > 0 && !new_work->merkle_branches)) {
        ESP_LOGE(TAG, "Memory allocation failed while parsing mining.notify");
...
static bool parse_set_extranonce(cJSON *json, StratumApiV1Message *message)
{
    ...
    if (strlen(extranonce1->valuestring) > MAX_EXTRANONCE_STR_LEN) {
        ESP_LOGE(TAG, "extranonce1 in set_extranonce is %u chars, max %d", (unsigned) strlen(extranonce1->valuestring),
                 MAX_EXTRANONCE_STR_LEN);
        return false;
    }
    if (message->extranonce_str) free(message->extranonce_str);
    message->extranonce_str = strdup(extranonce1->valuestring);
    if (!message->extranonce_str) {
        ESP_LOGE(TAG, "Out of memory copying extranonce1");
        return false;
    }
...
static bool parse_show_message(cJSON *json, StratumApiV1Message *message)
{
    ...
    if (!message->show_message) {
        ESP_LOGE(TAG, "Out of memory copying show_message");
        return false;
    }
...
static bool parse_subscribe_result(cJSON *json, StratumApiV1Message *message)
{
    ...
    if (strlen(extranonce->valuestring) > MAX_EXTRANONCE_STR_LEN) {
        ESP_LOGE(TAG, "extranonce1 in subscribe result is %u chars, max %d", (unsigned) strlen(extranonce->valuestring),
                 MAX_EXTRANONCE_STR_LEN);
        return false;
    }
    if (message->extranonce_str) free(message->extranonce_str);
...
static bool parse_result(cJSON *json, StratumApiV1Message *message)
{
    ...
    if (error && cJSON_IsArray(error) && cJSON_GetArraySize(error) >= 2) {
        ...
        if (cJSON_IsString(error_msg)) {
            ...
            ESP_LOGI(TAG, "Result failed: %s", message->error_str ? message->error_str : "(out of memory)");
            return true;
        }
