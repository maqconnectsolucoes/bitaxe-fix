 ...
 * @param data_len Total length of data, so a marker byte claiming more length
 *                 bytes than remain (untrusted/pool-supplied data) can't read past it
 * @param offset Pointer to current offset, will be updated after reading
 * @return Decoded varint value (0 if the buffer didn't have room for the full varint)
 */
