...
double test_nonce_value(const bm_job *job, const uint32_t nonce, const uint32_t rolled_version, const uint8_t midstate_index)
{
    // construct_bm_job() hashes the first 64 bytes of the header (version +
    // prev_block_hash + the first 28 bytes of merkle_root) into job->midstate
    // (or midstate1/2/3, one per version-rolled variant). When the caller
    // knows which of those the ASIC used, resume the SHA-256 from that saved
    // midstate instead of rehashing those 64 bytes on every nonce - only the
    // remaining 16 bytes (last 4 bytes of merkle_root + ntime + target +
    // nonce) need to be processed.
    const uint8_t *midstate = NULL;
    if (midstate_index < job->num_midstates) {
        switch (midstate_index) {
            case 0: midstate = job->midstate; break;
            case 1: midstate = job->midstate1; break;
            case 2: midstate = job->midstate2; break;
            case 3: midstate = job->midstate3; break;
        }
    }

    uint8_t merkle_root_reversed[32];
    reverse_32bit_words(job->merkle_root, merkle_root_reversed);

    uint8_t tail[16];
    memcpy(tail, merkle_root_reversed + 28, 4);   // last 4 bytes of reversed merkle_root
    ...
    if (midstate != NULL) {
        ...
        sha256_resume_bin(midstate, tail, sizeof(tail), first_hash);
