...
task_result *BM1397_process_work(void *pvParameters)
{
    ...
    if (!asic_result.is_job_response) {
        // register_address is a raw byte from the ASIC over UART; a corrupted byte that
        // still passes CRC can exceed the sparse REGISTER_MAP's actual size and read OOB.
        if (asic_result.cmd.register_address >= sizeof(REGISTER_MAP) / sizeof(REGISTER_MAP[0])) {
            ESP_LOGW(TAG, "Invalid register address from ASIC, out of range: %02x", asic_result.cmd.register_address);
            return NULL;
        }
    ...
    // active_jobs/valid_jobs are allocated with 128 entries (create_jobs_task.c); a corrupted
    // UART byte from the ASIC can otherwise push rx_job_id up to 252 and index out of bounds.
    if (rx_job_id >= 128) {
        ESP_LOGW(TAG, "Invalid job id from ASIC, out of range: %d", rx_job_id);
        return NULL;
    }
    ...
    result.midstate_index = rx_midstate_index;
