...
task_result * BM1366_process_work(void * pvParameters)
{
    ...
    // UART byte from the ASIC can otherwise push job_id up to 248 and index out of bounds.
    if (job_id >= 128) {
        ESP_LOGW(TAG, "Invalid job id from ASIC, out of range: %d", job_id);
        return NULL;
    }
    ...
    result.midstate_index = MIDSTATE_INDEX_NONE;
