...
{
    asic_range_t range = {.min = asic_options_min(options), .max = asic_options_max(options)};

    if (overclock && oc_max != 0) {
        if (range.max == 0 || oc_max > range.max) {
            range.max = oc_max;
        }
        if (range.min == 0 || (oc_min != 0 && oc_min < range.min)) {
            range.min = oc_min;
        }
    }
    return range;
